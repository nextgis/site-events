...
const getEvents = ({ isHappened, sorting, locale }: getEventsOptions) => {
  ...
  return axios
    .get(
      `${baseUrl}/wp-json/ng/v1/events?happened=${isHappened}&sorting=${sorting}`
    )
    .then((resp) =>
      resp.data?.map((event: EventFromApi) => formatEvent(event))
    );
};
...
const getEvent = async (id: number, locale: string) => {
  const baseUrl = getBaseApiUrl(locale);
  return axios
    .get(`${baseUrl}/wp-json/ng/v1/event/${id}`)
    .then((resp) => resp.data && formatEvent(resp.data));
};

const subscribeToEvent = (id: number, userData: UserData, locale: string) => {
  const baseUrl = getBaseApiUrl(locale);
  return axios
    .post(`${baseUrl}/wp-json/ng/v1/event/${id}/subscribe`, userData)
    .then((resp) => resp.data);
};
