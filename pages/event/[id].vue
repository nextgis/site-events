<template>
  <div class="app-page" v-if="event">
    <header
      class="event-header bg-dark text-white q-mx-nlg q-px-lg q-py-xl q-mb-xl q-dark"
    >
      ...
      <div class="event-header__inner">
        ...
        <section class="event-header__meta q-mt-lg">
          ...
          >
            ...
            <div
              class="event-header__info-address text-body2"
              v-if="event.address"
            >
              <q-chip
    ...
    <div class="event-content row q-col-gutter-lg">
      ...
      <div class="col-12 col-md-5 col-lg-4 offset-md-1">
        ...
        <q-card
          class="bg-grey-2"
          flat
          v-if="!isHappened && event.type !== 'outer'"
        >
        ...
        <q-card
          class="bg-grey-2"
          flat
          v-else-if="event.video || event.presentation"
        >
...
const {
  ...
  getEvent(Number(route.params.id), locale.value)
);
...
const videoUrl: ComputedRef<string | undefined> = computed(() => {
  if (event.value && event.value.video) {
    const eventVideoWithoutSlash = event.value.video.endsWith("/")
      ? event.value.video.slice(0, -1)
      : event.value.video;
    ...
    if (
      eventVideoUrl.includes("www.youtube.com") ||
      eventVideoUrl.includes("youtu.be")
    )
      return `https://www.youtube.com/embed/${videoId}`;

    if (eventVideoUrl.includes("rutube.ru"))
